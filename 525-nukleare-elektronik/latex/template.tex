...
\def\ba{$\ce{^133Ba}$ }
\def\na{$\ce{^22Na}$ }
\def\caesium{$\ce{^133Cs}$ }
\def\neon{$\ce{^22Ne}$ }

\def\fig#1{Abbildung \ref{fig:#1}}
\def\tab#1{Tabelle \ref{tab:#1}}
\def\eq#1{Gleichung \eqref{eq:#1}}
\def\sec#1{Abschnitt \eqref{sec:#1}}
...
\DeclareSIUnit\bin{\text{Bin}}
\DeclareSIUnit\count{\text{Anzahl}}
\DeclareSIUnit\unity{}

%\titlehead{\centering\includegraphics[width=0.6\textwidth]{fig name here}} %for a title picture
\graphicspath{{../figs/}}
