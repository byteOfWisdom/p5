\chapter{Analyse der Bodenprobe}\label{sec:bodenprobe}
...
Die den Linien zugeordneten Radionuklide lassen sich in Elemente aus der Uran-Radium-Reihe ($\ce{^214Pb}$ und $\ce{^214Bi}$) \cite[3]{list_bmumwelt}, natürliche Radionuklide ($\ce{^40K}$ \cite[55]{ex_techniques},\cite[13]{list_bmumwelt}), kosmogene Radionuklide $\ce{^22Na}$ und $\ce{^7Be}$ \cite{be7}) und künstliche Radionuklide ($\ce{^137Cs}$). Dies entspricht den Erwartungen, was in einer zufälligen Bodenprobe aus der Umgebung gefunden werden kann. Der gefundene, deutlich trennbare Peak bei $\SI{1896(23)}{\kilo\eV}$ konnte bei der Zuordnung keinem sinnvollem Element zugeordnet werden, d.h. keinem Element, was in einer Bodenprobe vorkommen könnte (nicht nur künstlich für sehr kurze Halbwertszeiten im Labor erzeugt wird) und keinem Übergang, der eine signifikante Intensität (mindestens $1\%$, da der Peak eine große Fläche hat, also der Übergang relativ wahrscheinlich sein muss) hat. Ansonsten hat die Zuordnung gut funktioniert und erscheint plausibel.
