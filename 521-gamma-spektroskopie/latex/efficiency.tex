...
\begin{table}[h]
	...
	\hline
    \end{tabular}
    \caption{Halbwertszeiten \todo{source} der Isotope sowie Aktivitäten der Proben}
    \label{tab:fwhm}
\end{table}
...
\begin{align}
	N_{E_\gamma, \text{Detektor}} = A(t) \cdot T_\text{Messung} \cdot q = A_0 e^{-\frac{t \ln(2)}{T_{1/2}}} T_\text{Messung} \frac12 \bigg (1 - \sqrt{1 + (\frac{r_\text{Detektor}}{d_\text{Probe}})}^{-1}\bigg)
\end{align}
...
Mit diesen Überlegungen wird für die Cäsium\-137 Quelle die Effizienz in beiden Detektoren bestimmt.
