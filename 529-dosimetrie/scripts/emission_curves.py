...
t0 = 273
t = p.ev(weather["air_temp"] + 273, 1.5)
p0 = 1013
air_density = rho_0 * (t0 / t) * (pressure / p0)
air_mass = air_volume * air_density
print("density is: ", air_density.format())
print("mass is: ", air_mass.format())
...
def plot_measurement(measurement):
    ...
    x_var, amp_voltage = data[0], data[1] * 1e3
...
cu_res_ev = p.ev(cu_res, cu_err)
print("Kupfer fit: a = ", cu_res_ev[0].format(), ", b = ", cu_res_ev[1].format())
...
mo_res_ev = p.ev(mo_res, mo_err)
print("Molybdän fit: a = ", mo_res_ev[0].format(), ", b = ", mo_res_ev[1].format())
...
dose_y = plt.gca().secondary_yaxis("right",functions=(lambda x: 32.4 * x, lambda x: x / 32.4))
dose_y.set_ylabel("Äquivalentsdosisleistung / $mSvs^{-1}$")
std.default.plt_finish("Beschleunigungsspannung $U_B$ / V",  "mittlere Ionisationsdosisleistung <j> / $mAkg^{-1}$")

# %%
heating_current_cu, ion_current_cu, ion_dose_rate_cu = plot_measurement("current_other_1")
cu_res, (cu_err, cu_rsq) = std.curve_fit(std.linear, heating_current_cu, ion_dose_rate_cu)
cu_res_ev = p.ev(cu_res, cu_err)
print("Kupfer fit: a = ", cu_res_ev[0].format(), ", b = ", cu_res_ev[1].format())
std.default.plt_func(std.linear, cu_res, f"$R^2= {round(cu_rsq, 3)}$", alpha=0.25)
heating_current_mo, ion_current_mo, ion_dose_rate_mo = plot_measurement("current_other_2")
mo_res, (mo_err, mo_rsq) = std.curve_fit(std.linear, heating_current_mo, ion_dose_rate_mo)
mo_res_ev = p.ev(mo_res, mo_err)
print("Molybdän fit: a = ", mo_res_ev[0].format(), ", b = ", mo_res_ev[1].format())
std.default.plt_func(std.linear, mo_res, f"$R^2 = {round(mo_rsq, 3)}$", alpha=0.25)
...
std.print_tex_table(cu_current_table, "../latex/cu_current_other_device.table")
...
std.print_tex_table(mo_current_table, "../latex/mo_current_other_device.table")
